...
class CumotionRobotSegmenter(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "depth_image_topics", ["/cumotion/depth_1/image_raw"]
        )
        self.declare_parameter(
            "depth_camera_infos", ["/cumotion/depth_1/camera_info"]
        )
        self.declare_parameter(
        ...
        self.declare_parameter(
            "update_link_sphere_server", "segmenter_attach_object"
        )
        ...
        try:
            ...
            self.__yml_path = (
                self.__yml_path.get_parameter_value().string_value
            )
            if self.__yml_path == "":
        ...
        distance_threshold = (
            self.get_parameter("distance_threshold")
            .get_parameter_value()
            .double_value
        )
        time_sync_slop = (
        ...
        debug_robot_topic = (
            self.get_parameter("debug_robot_topic")
            .get_parameter_value()
            .string_value
        )
        depth_image_topics = (
        ...
        self.approx_time_sync.registerCallback(
        ...
        self.mask_publishers = [
            self.create_publisher(Image, topic, mask_qos)
            for topic in publish_mask_topics
        ]
        self.segmented_publishers = [
            self.create_publisher(Image, topic, world_depth_qos)
        ...
        self.get_logger().info(
            f"Node initialized with {self._num_cameras} cameras"
        )
    ...
    def publish_robot_spheres(self, traj: CuJointState):
        kin_state = self._cumotion_segmenter.robot_world.get_kinematics(
            traj.position
        )
    ...
    def is_subscribed(self) -> bool:
        count_mask = max(
            [
                mask_pub.get_subscription_count()
                for mask_pub in self.mask_publishers
            ]
            + [
                seg_pub.get_subscription_count()
                for seg_pub in self.segmented_publishers
            ]
        )
        if count_mask > 0:
            return True
    ...
    def filter_depth_mask(self, robot_mask, depth_image):
        ...
        combined_mask = (
            np.logical_or(robot_mask, invalid_depth_mask).astype(np.uint8)
            * 255
        )
    ...
    def publish_images(
        self, depth_masks, segmented_depth_images, camera_header, idx: int
    ):
        ...
        if self.segmented_publishers[idx].get_subscription_count() > 0:
            ...
            msg = self.br.cv2_to_imgmsg(
                segmented_depth, self._depth_encoding[idx]
            )
            msg.header = camera_header[idx]
    ...
    def on_timer(self):
        ...
        if (
            (
                not all(
                    ...
                    for intrinsic in self._depth_intrinsics
                )
            )
            or (len(self._camera_headers) == 0)
                    ...
                    try:
                        t = self.tf_buffer.lookup_transform(
                            ...
                            rclpy.duration.Duration(
                                seconds=self._tf_lookup_duration
                            ),
                        )
        ...
        depth_image = self._tensor_args.to_device(
        ...
        if not self._cumotion_segmenter.ready:
            intrinsics = self._tensor_args.to_device(intrinsics).view(
        ...
        q = CuJointState.from_numpy(
            position=js, joint_names=j_names, tensor_args=self._tensor_args
        ).unsqueeze(0)
...
def main(args=None):
    ...
    try:
        ...
        cumotion_segmenter.get_logger().info(
            "Starting CumotionRobotSegmenter node"
        )
        rclpy.spin(cumotion_segmenter)
    except KeyboardInterrupt:
        cumotion_segmenter.get_logger().info(
            "Destroying CumotionRobotSegmenter node"
        )
