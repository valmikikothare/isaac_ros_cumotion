...
from isaac_ros_cumotion.update_kinematics import (
    UpdateLinkSpheresServer,
    get_robot_config,
)
...
class CumotionActionServer(Node):
    def __init__(self):
        ...
        try:
            ...
            self.__tool_frame = (
                self.__tool_frame.get_parameter_value().string_value
            )
        ...
        self.__add_ground_plane = (
            self.get_parameter("add_ground_plane")
            .get_parameter_value()
            .bool_value
        )
        ...
        self.__max_attempts = (
            self.get_parameter("max_attempts")
            .get_parameter_value()
            .integer_value
        )
        self.__num_graph_seeds = (
            self.get_parameter("num_graph_seeds")
            .get_parameter_value()
            .integer_value
        )
        self.__num_trajopt_seeds = (
            self.get_parameter("num_trajopt_seeds")
            .get_parameter_value()
            .integer_value
        )
        ...
        self.__read_esdf_grid = (
            self.get_parameter("read_esdf_world")
            .get_parameter_value()
            .bool_value
        )
        ...
        self.__grid_center_m = (
            self.get_parameter("grid_center_m")
            .get_parameter_value()
            .double_array_value
        )
        self.__max_publish_voxels = (
            self.get_parameter("max_publish_voxels")
            .get_parameter_value()
            .integer_value
        )
        self.__workspace_file_path = (
            self.get_parameter("workspace_file_path")
            .get_parameter_value()
            .string_value
        )
        self.__grid_size_m = (
            self.get_parameter("grid_size_m")
            .get_parameter_value()
            .double_array_value
        )
        ...
        self.__use_aabb_on_request = (
            self.get_parameter("use_aabb_on_request")
            .get_parameter_value()
            .bool_value
        )
        self.__publish_voxel_size = (
            self.get_parameter("publish_voxel_size")
            .get_parameter_value()
            .double_value
        )
        self.__voxel_size = (
        ...
        if path.exists(self.__workspace_file_path):
            ...
            self.__grid_center_m = get_grid_center(
                min_corner, self.__grid_size_m
            )
    ...
    def update_voxel_grid(self):
        ...
        # Get the AABB
        min_corner = get_grid_min_corner(
            self.__grid_center_m, self.__grid_size_m
        )
        aabb_min = Point()
        ...
        if not response.success:
            self.get_logger().info(
                "ESDF request failed, try again after few seconds."
            )
            return False
        ...
        if torch.max(esdf_grid.feature_tensor) <= (
            ...
            self.get_logger().error(
                "ESDF data is empty, try again after few seconds."
            )
            return False
    ...
    def execute_callback(self, goal_handle):
        ...
        if (
            ...
            or self.__override_moveit_scaling_factors
        ):
        ...
        if len(plan_req.goal_constraints[0].joint_constraints) > 0:
            ...
            goal_config = [
                ...
                for x in range(
                    len(plan_req.goal_constraints[0].joint_constraints)
                )
            ]
            goal_jnames = [
            ...
            goal_state = self.motion_gen.get_active_js(
                CuJointState.from_position(
                    position=self.tensor_args.to_device(goal_config).view(
                        1, -1
                    ),
                    joint_names=goal_jnames,
                )
            )
            goal_pose = self.motion_gen.compute_kinematics(
                goal_state
            ).ee_pose.clone()
        elif (
...
def main(args=None):
    ...
    try:
        ...
        cumotion_action_server.get_logger().info(
