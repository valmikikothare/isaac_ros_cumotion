...
bool CumotionInterface::solve(
  ...
  while (!action_client_->result_ready &&
         node_->now().seconds() - start_time.seconds() <
             kTimeoutIntervalInSeconds) {
  ...
  moveit::core::robotStateMsgToRobotState(
      action_client_->plan_response.trajectory_start, robot_state);
  result_traj->setRobotTrajectoryMsg(
      robot_state, action_client_->plan_response.trajectory[0]);
